    ...
    if(zeta_update == 1 ){
      for(ii=0; ii<5; ii++){
        for(j=0; j<p; j++){
          ...
          r(0) = R::rnorm( 0.0 , 1.0 );
          zeta_cand = zeta(j) + size_a*r(0)/10.0;
          // evaluate the current status
          rx_curr(0) = 0.0;
          a_sq_curr = 0.0;
          for(k=0; k<n; k++){
            aa = alpha(j) + u(k)*zeta(j) - alpha0;
            if( aa > 0.0 ){
              rx_curr(0) = rx_curr(0) + res(k)*X(k,j)*aa*w(j);
              theta_sj_curr(k) = w(j);
              a_sq_curr = a_sq_curr + X(k,j)*X(k,j)*w(j)*w(j);
            }else{
              theta_sj_curr(k) = 0.0;
          ...
          curr(0) = -0.5*(a_sq_curr - 2*rx_curr(0))/sig(0) - 0.5*zeta(j)*zeta(j);
          
          rx_cand(0) = 0.0;
          a_sq_cand = 0.0;
          for(k=0; k<n; k++){
            aa = alpha(j) + u(k)*zeta_cand - alpha0;
            if( aa > 0.0 ){
              rx_cand(0) = rx_cand(0) + res(k)*X(k,j)*aa*w(j);
              theta_sj_cand(k) = w(j);
              a_sq_cand = a_sq_cand + X(k,j)*X(k,j)*w(j)*w(j);
            }else{
              theta_sj_cand(k) = 0.0;
            }
          }
          //curr = -0.5*log(a_sq_curr + 1.0/tau_w) - 0.5*alpha(j)*alpha(j) + 0.5*rx_curr*rx_curr/(a_sq_curr + 1.0/tau_w)/sig(0);
          cand(0) = -0.5*(a_sq_cand - 2*rx_cand(0))/sig(0) - 0.5*zeta_cand*zeta_cand;
          aa = R::runif( 0.0, 1.0);
          ratio = cand(0) - curr(0);
          if(ratio > log(aa) ){
            zeta(j) = zeta_cand;
