...
for(iii in 1:1){
  ...
  BIVS_boston = function(i0, tr, xall, yall, X1_intr, N, BURN, i){
    ...
    for(h in 1:5){
      ...
      fit.RW = IVS_RW_naive000(y, X, N, N1=N,
                       BURN, alpha, zeta,
                      w, beta, z,
                      sig, n1=n, p1=p, tau_w=1,
                      tau_z=1, alpha0=a0, beta0=b0,
    ...
    for(v in 1:length(a0_cand)){
      ...
      for(h in 1:K){
        ...
        pmt = proc.time()[3]
        fit.RW = IVS_RW_simplest(y_tr, X_tr, N ,N1=N,BURN,alpha,zeta,w,beta, z,sig, n1=n, p1=p, tau_w=1,tau_z=1,
    ...
    for(h in 1:5){
      ...
      zeta= rep(0,p);#rep(1,p)
      sig = matrix(1,1,1)
      fit.RW = IVS_RW_simplest(y, X,  N, N1=N, BURN, alpha, zeta, w,
